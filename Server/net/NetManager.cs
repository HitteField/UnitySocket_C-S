...
class NetManager
{
    ...
    public static void StartLoop(int listenPort)
    {
        ...
        Tool.DebugLog("Server started up suuccessfully.");
    ...
    public static void ReadListenfd()
    {
        try
        {
            ...
            Tool.DebugLog("Accept new client: " + clientfd.RemoteEndPoint.ToString());
        ...
        {
            Tool.DebugWarning("Accept fail: " + ex.ToString());
        }
    }
    ...
    public static void ReadClientfd(Socket clientfd)
    {
        ...
        if(readBuff.Remain<=0)
        {
            ...
            if (readBuff.Remain <= 0)
            {
                Tool.DebugWarning("Client " + state.RemoteName + " receive fail, maybe msg length > buff capacity.");
                //关闭套接字
                Close(state);
                return;
            }
        ...
        {
            Tool.DebugError("Client " + state.RemoteName + " receive socket exception " + ex.ToString());
            Close(state);
            return;
        }
        ...
        if(count <=0)
        {
            Tool.DebugLog("Client " + state.RemoteName + " socket close.");
            Close(state);
            return;
        }
    ...
    public static void OnReceiveData(ClientState state)
    {
        ...
        if(protoName == "")
        {
            Tool.DebugError("Client " + state.RemoteName + " msg decode name fail.");
            Close(state);
        }
        ...
        if(protoName!="MsgPing") Tool.DebugLog("Receive " + protoName + " from client " + state.RemoteName + ".");
        if (mi != null) mi.Invoke(null, ob);
        else Tool.DebugWarning("Receive unknow msg " + protoName + " from client " + state.RemoteName + ".");
    ...
    public static void Send(ClientState state,MsgBase msg)
    {
        ...
        {
            Tool.DebugError("Fail sending msg on BeginSend() " + ex.ToString());
        }
    }

...
public class NetManager
{
    ...
    private static void InitState()
    {
        ...
        lastPingTime = Time.time;       //初始化心跳PING
        lastPongTime = Time.time;       //初始化心跳PONG
    ...
    public static void Connect(ConnectMethod connectMethod,string str,int port)
    {
        ...
        {
            Debug.LogWarning("Conect fail, already connected.");
            return;
        }
        if(isConnecting)
        {
            Debug.Log("Is connecting, plz waiting.");
            return;
        }
        //初始化成员
        InitState();
    ...
    private static void ConnectCallback(IAsyncResult ar)
    {
        ...
        {
            Debug.Log("Socket Connect Fail with reason: " + ex.ToString());
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        {
            FireEvent(NetEvent.Close, string.Empty);            //分发“关闭连接”事件
            Debug.LogWarning("Socket Receive Fail with reason: " + ex.ToString());  
        }
    }
    ...
    public static void OnReceiveData()
    {
        ...
        if (protoName == "")
        {
            Debug.LogWarning("OnReceiveData MsgBase.DecodeName fail");
            return;
        }
    ...
    private static void PingUpdate()
    {
        ...
        if(Time.time-lastPongTime >pintInterval*3)
        {
            Close();
        }
    }
